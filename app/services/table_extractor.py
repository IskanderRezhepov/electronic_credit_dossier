...
def _schedule_rows(document: ReadDocument) -> dict | None:
    ...
    # Deduplicate OCR repeats by date and numeric payload.
    unique: list[dict] = []
    seen: set[tuple] = set()
    for row in rows:
...
def extract_tables(document: ReadDocument, document_type: str) -> list[dict]:
    """Extract conservative structured tables without inventing missing cells."""
    tables: list[dict] = []
    if document_type == 'payment_schedule' or _looks_like_schedule(document.full_text):
