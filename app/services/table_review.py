...
def _row_checks(table_name: str, rows: list[dict]) -> list[dict]:
    ...
    if table_name == "asset_vin_rows":
        ...
        for index, row in enumerate(rows):
            ...
            if previous_balance is not None and isinstance(principal, (int, float)) and isinstance(balance, (int, float)):
                expected_balance = float(previous_balance) - float(principal)
                difference = round(float(balance) - expected_balance, 2)
                checks.append({
                    "row": index + 1,
                    "check": "Остаток = предыдущий остаток − основной долг",
                    "valid": abs(difference) <= 1.0,
                    "difference_kzt": difference,
                    "message": "Остаток сходится." if abs(difference) <= 1.0 else f"Расхождение остатка {difference:,.2f} тенге.",
                })
