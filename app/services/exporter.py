...
def save_excel(data: dict, path: Path) -> None:
    ...
    for document in data['documents']:
        ...
        sheet.append(['Поле', 'Значение', 'Исходное значение', 'Страница', 'Метод', 'Уверенность', 'Статус', 'Проверено', 'Примечание', 'Цитата'])
        for cell in sheet[1]:
            cell.fill = header_fill
            cell.font = header_font
        ...
        for item in sorted_fields:
            ...
            sheet.append([
                item['label_ru'], value, original_value, item['page'], item['extraction_method'],
                item['confidence'], item.get('status'), item.get('reviewed_at'),
                item.get('notes'), item['quote'],
            ])
        for column, width in {'A':35,'B':42,'C':42,'D':11,'E':14,'F':13,'G':16,'H':24,'I':38,'J':90}.items():
