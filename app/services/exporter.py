...
def save_excel(data: dict, path: Path) -> None:
    ...
    for document in data['documents']:
        ...
        sheet.append(['Поле', 'Категория', 'Значение', 'Исходное значение', 'Страница', 'Метод', 'Уверенность', 'Статус', 'Проверено', 'Примечание', 'Цитата'])
        for cell in sheet[1]:
            cell.fill = header_fill
            cell.font = header_font
        ...
        for item in sorted_fields:
            ...
            sheet.append([
                item['label_ru'], item.get('category'), value, original_value,
                item['page'], item['extraction_method'], item['confidence'],
                item.get('status'), item.get('reviewed_at'),
                item.get('notes'), item['quote'],
            ])
        for column, width in {'A':35,'B':18,'C':42,'D':42,'E':11,'F':14,'G':13,'H':16,'I':24,'J':38,'K':90}.items():
