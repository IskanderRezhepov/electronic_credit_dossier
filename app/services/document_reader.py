    ...
    with fitz.open(path) as doc:
        for index, page in enumerate(doc, start=1):
            ...
            if _digital_text_is_usable(digital, min_digital_chars):
                pages.append(PageContent(index, digital, 'digital', len(digital), round(_digital_text_quality(digital), 2), {'digital': digital}))
                continue
            ...
            pages.append(PageContent(index, combined, method, len(combined), ocr['quality'], ocr['variants'], bool(ocr.get('cache_hit'))))
...
def _read_image(path: Path, languages: str, analysis_mode: str = 'standard') -> ReadDocument:
    ...
    return ReadDocument(path.name, 1, 'image', [PageContent(1, text, 'ocr', len(text), ocr['quality'], ocr['variants'], bool(ocr.get('cache_hit')))])
