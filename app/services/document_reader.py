...
def _choose_auto_ocr(image: Image.Image, languages: str, dpi: int,
    ...
    needs_stronger = (
        len(fast_text) < 180
        or fast_quality < 0.62
        or len(fast.get("layout_words") or []) < 18
    )
    if not needs_stronger:
        return fast, "auto-fast"

    accurate = _ocr_cached(image, languages, max(dpi, 230), cache_dir, "accurate")
    accurate_text = normalize_text(accurate.get("preferred", ""))
    accurate_quality = float(accurate.get("quality") or 0)

    fast_score = fast_quality + min(len(fast_text), 1200) / 5000
    accurate_score = accurate_quality + min(len(accurate_text), 1200) / 5000
    if accurate_score >= fast_score:
        return accurate, "auto-accurate"
    return fast, "auto-fast"
    ...
    with fitz.open(path) as doc:
        for index, page in enumerate(doc, start=1):
            ...
            if analysis_mode == "auto":
                ocr, page_profile = _choose_auto_ocr(image, languages, dpi, cache_dir)
            else:
            ...
            pages.append(PageContent(index, combined, method, len(combined), ocr['quality'], ocr['variants'], bool(ocr.get('cache_hit')), ocr.get('layout_words', []), float(image.width), float(image.height), page_profile))
