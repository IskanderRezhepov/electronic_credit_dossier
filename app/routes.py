...
from .services.regression_fixes import (
    override_document_type, postprocess_fields, postprocess_tables,
)

bp = Blueprint('main', __name__)

ANALYSIS_MODES = {
    "auto": {"label_ru": "Автоматический по страницам", "dpi": 180},
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for document_index, uploaded in enumerate(uploaded_files):
                ...
                corrected_type = override_document_type(read_result, classification.key)
                if corrected_type != classification.key:
                    classification.key = corrected_type
                    classification.label_ru = {
                    ...
                    }.get(corrected_type, classification.label_ru)
                ...
                fields = improve_fields(read_result, classification.key, fields, tables)
                tables = _normalise_tables(postprocess_tables(read_result, classification.key, fields, tables))
                fields = postprocess_fields(read_result, classification.key, fields, tables)
...
def reprocess_page(result_id: str, document_index: int, page_number: int):
    ...
    new_fields = improve_fields(read_result, classification.key, new_fields, new_tables)
    new_tables = _normalise_tables(postprocess_tables(read_result, classification.key, new_fields, new_tables))
    new_fields = postprocess_fields(read_result, classification.key, new_fields, new_tables)
