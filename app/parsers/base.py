...
def valid_contract_number(value: str) -> bool:
    """Validate contract/document numbers without rejecting compact IDs such as KAZ14112024KTL."""
    if not (5 <= len(value) <= 50):
        return False
    if not re.search(r'\d', value):
        return False
    # Typical slash/dash numbers are accepted directly. Compact identifiers must contain both letters and digits.
    if '/' in value or '-' in value:
        return True
    return bool(len(value) >= 8 and re.search(r'[A-ZА-Я]', value) and re.search(r'\d', value))
