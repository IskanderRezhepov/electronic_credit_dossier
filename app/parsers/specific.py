...
def _schedule_principal_fallback(document: ReadDocument) -> dict | None:
    """
    For scanned schedules, calculate the original principal from the first
    payment row: principal repayment + remaining principal balance.
    """
    date_pattern = re.compile(r"\b\d{2}[.\-/]\d{2}[.\-/]\d{2,4}\b")
    for page in document.pages[:2]:
        for date_match in date_pattern.finditer(page.text):
            window = page.text[date_match.end():date_match.end() + 420]
            values = []
            for match in re.finditer(
                r"\d{1,3}(?:[ \u00a0]\d{3})+(?:[,.]\d{1,2})",
                window,
                ...
                if parsed is not None:
                    values.append((parsed, match))
            if len(values) < 3:
                continue

            # In a standard schedule row the largest value is the remaining
            # balance; a repeated medium value is the principal repayment.
            balance = max(value for value, _ in values)
            smaller = [value for value, _ in values if value < balance]
            if not smaller:
                continue
            ...
            if total < 10000 or total <= balance:
                continue
            ...
            return field(
                ...
                page=page.page_number,
                quote=page.text[
                    max(0, date_match.start() - 120):
                    min(len(page.text), date_match.end() + 420)
                ],
                confidence=0.76 if page.extraction_method != "digital" else 0.86,
                extraction_method=page.extraction_method,
                value_type="calculated",
                status="candidate",
                notes=(
                    "Рассчитано по первой строке графика: погашение основного "
                    "долга плюс остаток после платежа. Требуется сверка."
                ),
            )
    return None
...
def _filename_act_references(document: ReadDocument) -> tuple[dict | None, dict | None]:
    ...
    act_match = re.search(r"Акт[^\n]{0,80}?(?:№|N)\s*(\d{1,4})", stem, re.I)
    if act_match:
        act = field(
            name="act_number",
            label_ru="Номер акта",
            value=act_match.group(1),
            page=None,
    ...
    linked_match = re.search(
        r"(?:лизинг\s*)?(?:№|N)\s*([A-ZА-Я0-9]+(?:[-_/][A-ZА-Я0-9]+){2,6})",
        stem,
        re.I,
    )
    if linked_match:
        value = normalize_contract_number(linked_match.group(1))
