...
def parse_purchase_contract(document: ReadDocument) -> list[dict]:
    ...
    _append(fields, find_first(document, patterns=[
        ...
        r'(?:ОБЩАЯ|ПОЛНАЯ)\s+(?:СТОИМОСТЬ|ЦЕНА)[^\d]{0,120}([\d\s]+(?:[,.]\d{1,2})?)\s*(?:ТЕНГЕ|KZT)',
        r'НА\s+ОБЩУЮ\s+СУММУ[^\d]{0,100}([\d\s]+(?:[,.]\d{1,2})?)\s*(?:ТЕНГЕ|KZT)',
...
def parse_payment_schedule(document: ReadDocument) -> list[dict]:
    ...
    if not lease_number:
        lease_number = filename_number(
            document,
            r'([A-ZА-Я]{1,5}\d*(?:[/_-][A-ZА-Я0-9]+){2,7})',
