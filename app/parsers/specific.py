...
def parse_payment_schedule(document: ReadDocument) -> list[dict]:
    ...
    _append(fields, find_first(document, patterns=[r'(?:Сумма займа|Сумма транша)\s*[:/]\s*(\d[\d\s]*(?:[,.]\d{1,2})?)'], name='loan_amount_kzt', label_ru='Сумма займа / транша, тенге', converter=parse_money, pages=2))
