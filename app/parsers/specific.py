...
def parse_purchase_contract(document: ReadDocument) -> list[dict]:
    ...
    _append(fields, find_first(document, patterns=[
        r'Общая (?:стоимость|цена)(?: настоящего)? (?:Договора|Оборудования)\s*(?:составляет|:)\s*([\d\s]+(?:[,.]\d{1,2})?)\s*(?:тенге|KZT)',
        r'Цена Оборудования[^\n]{0,100}?составляет\s*([\d\s]+(?:[,.]\d{1,2})?)\s*тенге',
        r'БАРЛЫҒЫ\s*/\s*ИТОГО\s*[:\-]?\s*([\d\s]+(?:[,.]\d{1,2})?)',
...
def parse_acceptance_act(document: ReadDocument) -> list[dict]:
    ...
    if not act_number:
        m = re.search(r'Акт[^\n]{0,160}?№\s*([A-ZА-Я0-9./_-]+)', document.filename, re.I)
        if m:
    ...
    if vins:
        preview = vins[:20]
        notes = None if len(vins) <= 20 else f'Показаны первые 20 из {len(vins)} VIN; полный список доступен в JSON/Excel.'
        fields.append(field(name='asset_vins', label_ru='VIN по акту', value=preview, page=None, quote=None,
                            confidence=0.68 if document.used_ocr else 0.93,
                            extraction_method='mixed' if document.used_ocr else 'digital', status='candidate', notes=notes))
...
def parse_payment_schedule(document: ReadDocument) -> list[dict]:
    fields: list[dict] = []
    lease_number = find_first(document, patterns=[
        r'к\s*Договору\s+финансового\s+лизинга[^\n]{0,80}?(?:№|N)\s*([A-ZА-Я0-9][A-ZА-Я0-9./_-]{4,40})',
        r'Договор(?:у|а)?\s+финансового\s+лизинга[^\n]{0,80}?(?:№|N)\s*([A-ZА-Я0-9][A-ZА-Я0-9./_-]{4,40})',
        r'Қаржылық\s+лизинг\s+шартына.{0,100}?(?:№|N)\s*([A-ZА-Я0-9][A-ZА-Я0-9./_-]{4,40})',
    ], name='lease_contract_number', label_ru='Номер договора лизинга', converter=normalize_contract_number, validator=valid_contract_number, pages=2)
    if not lease_number:
        lease_number = filename_number(document, r'([A-ZА-Я]{1,5}\d*(?:[/_-][A-ZА-Я0-9]+){2,6})',
